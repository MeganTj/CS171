...
void update_pendulum()
{
     ...
     * 
     */
    // Calculate the pendulum's momentum in the x-direction
    m1.px = m1.px - (m1.k * dt * m1.x) * 
            (1 - m1.rl / sqrt(pow(m1.x, 2) + pow(m1.y, 2)));
    // Update the pendulum's velocity in the x-direction
    float vx = m1.px / m1.m;
    m1.x = m1.x + dt * vx;
    // Calculate the pendulum's momentum in the y-direction
    m1.py = m1.py - (m1.k * dt * m1.y) * 
            (1 - m1.rl/ sqrt(pow(m1.x, 2) + pow(m1.y, 2))) + g * m1.m * dt;
    // Update the pendulum's velocity in the y-direction
    float vy = m1.py / m1.m;
    m1.y = m1.y + dt * vy;
