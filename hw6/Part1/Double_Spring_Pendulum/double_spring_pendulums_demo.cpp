...
void update_pendulums()
{
    ...
    float length = sqrt(pow(m1.x - m2.x, 2) + pow(m1.y - m2.y, 2));
    // Calculate the x and y momentums of the first mass
    m1.px = m1.px - (m1.k * dt * m1.x) * (1 - m1.rl / sqrt(pow(m1.x, 2) + 
        pow(m1.y, 2))) - (dt * m2.k) * (m1.x - m2.x) * (-m2.rl + 
        length) / length;

    m1.py = m1.py + dt * (g * m1.m + m1.k * m1.y * (-1 + m1.rl / (
        sqrt(pow(m1.x, 2) + pow(m1.y, 2)))) - m2.k * (-m2.rl + 
        length) * (m1.y - m2.y) / length);
    // Calculate the x and y momentums of the second mass
    m2.px = m2.px + (m2.k * dt) * (m1.x - m2.x) * 
            (length - m2.rl) / length; 

    m2.py = m2.py + dt * (g * m2.m + m2.k * (-m2.rl + 
        length) * (m1.y - m2.y) / length);
    // Update positions
    m1.x = m1.x + dt * (m1.px / m1.m);
    m1.y = m1.y + dt * (m1.py / m1.m);
    m2.x = m2.x + dt * (m2.px / m2.m);
    m2.y = m2.y + dt * (m2.py / m2.m);
